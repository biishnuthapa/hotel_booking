...
describe('transaction state reporting', () => {
  it('reports pending and finalized confirmations', async () => {
    ...
    await waitForTransaction({ hash: '0xpending', wait: vi.fn().mockResolvedValue(receipt) }, (state) => states.push(state.state))
    expect(states).toEqual(['pending', 'finalized'])
  })

  it('reports replacement before finalization', async () => {
    const states = []
    const replacement = { hash: '0xreplacement', wait: vi.fn().mockResolvedValue({ hash: '0xfinal' }) }
    const error = Object.assign(new Error('replaced'), {
      code: 'TRANSACTION_REPLACED',
      cancelled: false,
      replacement,
    })
    await waitForTransaction({ hash: '0xold', wait: vi.fn().mockRejectedValue(error) }, (state) => states.push(state.state))
