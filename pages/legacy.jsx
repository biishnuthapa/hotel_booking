      ...
      .catch((error) => {
        if (active) {
          setWalletRecords({
            bookings: [],
            tokens: [],
            error: error?.message || 'Unable to read wallet records.',
          })
        }
      })
    return () => { active = false }
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      <div className="rounded-2xl border border-amber-300 bg-amber-50 p-6 text-amber-950">
        ...
        <p className="mt-2 max-w-3xl text-sm">
          V1 has known design limitations and is not used for new writes. Listings, bookings, and
          NFTs are not imported into the current protocol.
        </p>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {listings.map((listing) => (
          <article key={listing.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <img
              src={normalizeIpfsUrl(listing.images?.[0] || '')}
              alt=""
      ...
      <section className="mt-10 border-t border-slate-200 pt-8">
        ...
        <p className="mt-1 text-sm text-slate-600">
          These bookings and NFTs are displayed without transaction controls.
        </p>
...
export async function getServerSideProps() {
  let chain
  try {
    chain = getChainConfig(ACTIVE_CHAIN_ID)
  } catch {
