...
import { encodeAuthorizationPayload } from '@/utils/checkInAuthorization'
...
export default function PropertyManagement() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [allListings, hostBookings, tokenInfo, withdrawal] = await Promise.all([
        getListings(ACTIVE_CHAIN_ID, { includeInactive: true }),
        getHostBookings(ACTIVE_CHAIN_ID, address),
      ...
      const [hydrated, disputeStates] = await Promise.all([
        Promise.all(
          owned.map(async (listing) => ({
            listing,
            rooms: (await getListingWithRooms(ACTIVE_CHAIN_ID, listing.id)).rooms,
          }))
        ),
        Promise.all(
          hostBookings
  ...
  const signAuthorization = async (bookingId) => {
    ...
    try {
      ...
      const deepLink = `${window.location.origin}/check-in?authorization=${encodeAuthorizationPayload(serialized)}`
  ...
  const updateRoomEdit = (roomId, field, value) => {
    setRoomEdits((current) => ({
      ...current,
      [roomId]: { ...(current[roomId] || {}), [field]: value },
    }))
  }
  ...
  const updateListingEdit = (listingId, field, value) => {
    setListingEdits((current) => ({
      ...current,
      [listingId]: { ...(current[listingId] || {}), [field]: value },
    }))
  }
  ...
  const addRoom = async (listingId) => {
    ...
    if (!token) return
    try {
      const atomicPrice = parseTokenAmount(draft.price, token.decimals)
      ...
      if (created) setRoomDrafts((current) => ({ ...current, [listingId]: null }))
    } catch (error) {
      toast.error(errorMessage(error))
    }
  }
  ...
  const updateRoomPrice = async (room) => {
    ...
    if (!value || Number(value) <= 0 || !token)
      return toast.error('Enter a valid higher or lower future price.')
    try {
      const atomic = parseTokenAmount(value, token.decimals)
      await run(
        'updateRoomTypePrice',
        [room.id, atomic],
        'Future room price updated.',
        `price-${room.id}`
      )
    } catch (error) {
      toast.error(errorMessage(error))
  ...
  const increaseRoomCapacity = async (room) => {
    ...
    if (!Number.isSafeInteger(value) || value <= Number(room.capacity))
      return toast.error('New capacity must be greater than current capacity.')
    await run(
      'increaseRoomTypeCapacity',
      [room.id, value],
  ...
  const increaseListingRooms = async (listing) => {
    ...
    if (!Number.isSafeInteger(value) || value <= Number(listing.totalRooms))
      return toast.error('New inventory must exceed current total rooms.')
    await run(
      'increaseListingRooms',
      [listing.id, value],
      'Listing inventory increased.',
      `listing-rooms-${listing.id}`
    )
  }
  ...
  return (
    <>
      <Head>
        <title>Host dashboard · HospitalityBooking</title>
      </Head>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        <PageHeader
          ...
          actions={
            <Link href="/manage/new" className="button-primary inline-flex">
              Create property
            </Link>
          }
        />
        ...
        {!configured && (
          <EmptyState
            ...
            description={`Add the finalized ${chain.name} addresses before loading host records.`}
          />
        )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-5">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-400">
                  Checked-in stay revenue remains escrowed through the dispute window.
                </p>
              </div>
              <button
                type="button"
        ...
        {!loading && configured && address && properties.length === 0 && (
          <EmptyState
            title="No properties owned by this wallet"
            description="Create a property, then add room types within its declared inventory."
        ...
        <section className="mt-7 grid gap-6">
          {properties.map(({ listing, rooms }) => {
            const draft = roomDrafts[listing.id] || {
              name: '',
              metadataURI: '',
              price: '',
              capacity: 1,
            ...
            return (
              <article key={listing.id} className="app-card overflow-hidden">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 p-5 sm:p-6">
                  <div>
                    <p className="eyebrow">Listing #{listing.id.toString()}</p>
                    <h2 className="mt-2 text-2xl font-semibold text-slate-950">{listing.name}</h2>
                    <p className="mt-2 text-sm text-slate-500">
                      {listing.allocatedCapacity.toString()} of {listing.totalRooms.toString()}{' '}
                      rooms allocated across {rooms.length} room types
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      run(
                        ...
                        listing.active ? 'Listing deactivated.' : 'Listing reactivated.',
                        `listing-active-${listing.id}`
                      )
                    }
                    className={
                      listing.active ? 'button-danger inline-flex' : 'button-secondary inline-flex'
                    }
                  >
                    {listing.active ? 'Deactivate listing' : 'Reactivate listing'}
                  </button>
                </div>
                    ...
                    {rooms.map((room) => (
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                              Room #{room.id.toString()} · capacity {room.capacity.toString()}
                            </p>
                          </div>
                          <span
                            className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${room.active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}
                          >
                        ...
                        <div className="mt-4 grid gap-2 sm:grid-cols-2">
                          <div>
                            <input
                              value={roomEdits[room.id]?.price || ''}
                              onChange={(event) =>
                                updateRoomEdit(room.id, 'price', event.target.value)
                              }
                              inputMode="decimal"
                              placeholder={`New ${token.symbol} price`}
                              className="field mt-0"
                            />
                            <button
                              type="button"
                              disabled={Boolean(busyAction)}
                              onClick={() => updateRoomPrice(room)}
                              className="button-secondary mt-2 inline-flex w-full"
                            >
                              Update price
                            </button>
                          </div>
                          <div>
                            <input
                              type="number"
                              min={Number(room.capacity) + 1}
                              value={roomEdits[room.id]?.capacity || ''}
                              onChange={(event) =>
                                updateRoomEdit(room.id, 'capacity', event.target.value)
                              }
                            ...
                            <button
                              type="button"
                              disabled={Boolean(busyAction)}
                              onClick={() => increaseRoomCapacity(room)}
                              className="button-secondary mt-2 inline-flex w-full"
                            >
                              Increase capacity
                            </button>
                        ...
                        <button
                          ...
                          onClick={() =>
                            run(
                              'setRoomTypeActive',
                              [room.id, !room.active],
                              room.active ? 'Room type deactivated.' : 'Room type reactivated.',
                              `room-active-${room.id}`
                            )
                          }
                          className="mt-3 text-xs font-semibold text-slate-600 hover:text-slate-950"
                        >
                          {room.active ? 'Deactivate room type' : 'Reactivate room type'}
                        </button>
                      </div>
                    ))}
                  </div>
                  ...
                  <details className="mt-5 rounded-2xl border border-slate-200 p-4">
                    ...
                    <div className="mt-4 grid gap-4 lg:grid-cols-2">
                      <div className="rounded-xl bg-slate-50 p-4">
                        <p className="text-sm font-semibold">Increase listing inventory</p>
                        <p className="mt-1 text-xs text-slate-500">
                          Current total: {listing.totalRooms.toString()}. Totals cannot decrease.
                        </p>
                        <div className="mt-3 flex gap-2">
                          <input
                            type="number"
                            min={Number(listing.totalRooms) + 1}
                            value={listingEdits[listing.id]?.totalRooms || ''}
                            onChange={(event) =>
                              updateListingEdit(listing.id, 'totalRooms', event.target.value)
                            }
                          ...
                          <button
                            type="button"
                            disabled={Boolean(busyAction)}
                            onClick={() => increaseListingRooms(listing)}
                      ...
                      <div className="rounded-xl bg-slate-50 p-4">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          Unallocated listing capacity: {availableCapacity}
                        </p>
                        <div className="mt-3 grid gap-2 sm:grid-cols-2">
                          <input
                            value={draft.name}
                            onChange={(event) =>
                              updateRoomDraft(listing.id, 'name', event.target.value)
                            }
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              updateRoomDraft(listing.id, 'metadataURI', event.target.value)
                            }
                          ...
                          <input
                            value={draft.price}
                            onChange={(event) =>
                              updateRoomDraft(listing.id, 'price', event.target.value)
                            }
                            inputMode="decimal"
                            placeholder={`Price in ${token.symbol}`}
                            className="field mt-0"
                          />
                          <div className="flex gap-2">
                            <input
                              ...
                              onChange={(event) =>
                                updateRoomDraft(listing.id, 'capacity', event.target.value)
                              }
                              className="field mt-0"
                            />
                            <button
                              type="button"
                              disabled={availableCapacity < 1 || Boolean(busyAction)}
                              onClick={() => addRoom(listing.id)}
        ...
        <section className="mt-12">
          <div className="mb-5">
            <p className="eyebrow">Arrival operations</p>
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">
              Bookings requiring host attention
            </h2>
          </div>
          {!loading && configured && address && bookings.length === 0 && (
            <EmptyState
              title="No host bookings yet"
              description="New bookings for your properties will appear here."
            />
          )}
            ...
            {bookings.map((booking) => {
              ...
              return (
                <article key={id} className="app-card p-5 sm:p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        Guest <AddressLink chainId={chain.id} address={booking.guest} /> ·{' '}
                  ...
                  {status === 'Booked' && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        type="button"
                        disabled={Boolean(busyAction)}
                        onClick={() => signAuthorization(id)}
                        className="button-primary inline-flex"
                      >
                        {busyAction === `sign-${id}` ? 'Signing…' : 'Sign guest check-in'}
                      </button>
                      <button
                        type="button"
                        disabled={Boolean(busyAction)}
                        onClick={() =>
                          run(
                            ...
                            [booking.id],
                            'Outstanding authorizations revoked.',
                            `revoke-${id}`
                          )
                        }
                        className="button-secondary inline-flex"
                  ...
                  {status === 'CheckedIn' && (
                    <div className="mt-4 rounded-2xl border border-teal-200 bg-teal-50 p-4 text-sm text-teal-950">
                      <p>
                        Host payout remains escrowed through {formatTimestamp(disputeWindowEnd)}.
                      </p>
                      {canComplete && (
                        <button
                          type="button"
                    ...
                    <div className="mt-4 grid gap-4 rounded-2xl border border-teal-200 bg-teal-50 p-4 sm:grid-cols-[1fr_190px]">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-wide text-teal-900">
                          Guest-bound authorization
                        </p>
                        <textarea
                          readOnly
                          value={authorization.serialized}
                          className="field mt-2 h-40 font-mono text-[10px]"
                        />
                        <div className="mt-2 flex flex-wrap gap-2">
                          <button
                            type="button"
                            onClick={() => copyText(authorization.serialized, 'Authorization')}
                            className="button-secondary inline-flex"
                          >
                            Copy JSON
                          </button>
                          <button
                            type="button"
                            onClick={() => copyText(authorization.deepLink, 'Check-in link')}
                            className="button-secondary inline-flex"
                          >
                            Copy link
                          </button>
                      ...
                      <div>
                        <img
                          src={authorization.qrDataURL}
                          alt={`Guest check-in QR code for booking ${id}`}
                          className="w-full rounded-2xl border border-teal-200 bg-white p-2"
                        />
                        <a
                          href={authorization.deepLink}
                          className="mt-2 block break-all text-xs font-semibold text-teal-800 underline"
                        >
                          Open check-in link
                        </a>
                      </div>
                  ...
                  {canOpenDispute && (
                    <details className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                      ...
                      <p className="mt-2 text-xs leading-5 text-amber-900">
                        Required bond:{' '}
                        {token ? formatTokenAmount(bond, token.decimals, token.symbol) : '—'}. A
                        loss or timeout forfeits it to the guest.
                      </p>
                      <input
                        value={evidence[id] || ''}
                        onChange={(event) =>
                        ...
                        placeholder="ipfs:// evidence or immutable reference"
                        className="field"
                      />
                      {evidence[id] && (
                  ...
                  {status === 'Disputed' && dispute && (
                    <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
                      <p className="font-semibold">Dispute settlement is frozen.</p>
                      <p className="mt-1 text-xs">
                        Arbitrator deadline: {formatTimestamp(dispute.deadline)}
                      </p>
                      <code className="mt-2 block break-all text-[10px]">
                        Evidence: {dispute.evidenceHash}
                      </code>
                      {canTimeout && (
                        <button
                          type="button"
                          ...
                          onClick={() =>
                            run(
                              ...
                              [booking.id],
                              'Timed-out dispute resolved in the guest’s favor.',
                              `timeout-${id}`
                            )
                          }
                          className="button-danger mt-3 inline-flex"
