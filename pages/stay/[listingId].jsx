...
import { approveAndBook, getListingWithRooms, getPaymentTokenInfo } from '@/services/blockchain'
...
export default function StayPage({ listing, rooms, token, chain, loadError, todayUTC }) {
  ...
  const minimumCheckIn = addDaysToDateString(todayUTC, 1)
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        <EmptyState
          ...
          description={loadError || 'The deployment is not configured.'}
          actionHref="/"
  ...
  return (
    <>
      <Head>
        <title>{listing.name} · HospitalityBooking</title>
        ...
        <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1fr)_400px]">
          <div className="min-w-0">
            ...
            <div className="mt-7 flex flex-wrap items-start justify-between gap-4">
              ...
              <span
                className={`rounded-full px-3 py-1.5 text-xs font-bold ${listing.active ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}
              >
            ...
            <section className="mt-8">
              ...
              {rooms.length === 0 ? (
                <EmptyState
                  ...
                  description="The host has not added room inventory yet."
                />
              ) : (
                <div className="mt-4 grid gap-4">
                  {rooms.map((room) => (
                    <article
                      key={room.id}
                      className={`app-card flex flex-wrap items-center justify-between gap-4 p-5 ${!room.active ? 'opacity-60' : ''}`}
                    >
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          Up to {room.capacity} rooms per night
                        </p>
                      </div>
          ...
          <aside className="h-fit lg:sticky lg:top-28">
            <form onSubmit={submit} className="app-card p-6">
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">
                Book with {token.symbol}
              </h2>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Checkout is exclusive. Your exact price and schedule are snapshotted on-chain.
              </p>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (checkOutDate && checkOutDate <= next)
                        setCheckOutDate(addDaysToDateString(next, 1))
                    }}
                    className="field"
                    required
                  />
              ...
              {quote.error && checkInDate && checkOutDate && (
                <p className="mt-3 text-sm text-red-700">{quote.error}</p>
              )}
              {quote.total !== undefined && (
                <div className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm">
                  <div className="flex justify-between gap-3 text-slate-600">
                    <span>
                      {quote.nights} nights × {roomsRequested} rooms
                    </span>
                    <span>{formatTokenAmount(quote.base, token.decimals, token.symbol)}</span>
                  </div>
                  <div className="mt-2 flex justify-between gap-3 text-slate-600">
                    <span>Security deposit</span>
                    <span>{formatTokenAmount(quote.deposit, token.decimals, token.symbol)}</span>
                  </div>
                  <div className="mt-3 flex justify-between gap-3 border-t border-slate-200 pt-3 font-semibold text-slate-950">
                    <span>Total escrow</span>
                    <span>{formatTokenAmount(quote.total, token.decimals, token.symbol)}</span>
                  </div>
                </div>
              )}
              ...
              <p className="mt-3 text-center text-xs leading-5 text-slate-500">
                ERC-20 approval is requested only when the current allowance is insufficient.
              </p>
            </form>
          </aside>
        </div>
  ...
  if (!isDeploymentConfigured(ACTIVE_CHAIN_ID)) {
    return {
      props: {
        ...
        chain: chainProps,
        todayUTC,
        loadError: `${chain.name} is not configured.`,
      },
    }
  }
  try {
    const [{ listing, rooms }, token] = await Promise.all([
      getListingWithRooms(ACTIVE_CHAIN_ID, params.listingId),
