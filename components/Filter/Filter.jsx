    ...
    <div className="flex flex-wrap justify-center gap-6">
      {filterOptions.map((filter, index) => (
        <div key={index} className="flex flex-col items-center cursor-pointer" onClick={() => toggleFilter(filter.name)}>
          <FontAwesomeIcon icon={filter.icon} className={`text-xl ${selectedFilters.includes(filter.name) ? 'text-[#836FFF]' : 'text-white'}`}/>
          <span className={`text-sm font-medium ${selectedFilters.includes(filter.name) ? 'text-[#836FFF]' : 'text-white'}`} onClick={() => toggleFilter(filter.name)}>{filter.name}</span>
        </div>
      ))}
    </div>
  );
