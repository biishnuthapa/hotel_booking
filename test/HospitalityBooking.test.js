...
describe('HospitalityBooking', () => {
  ...
  it('accepts one guest-bound host EIP-712 authorization and rejects replay', async () => {
    ...
    await fixture.booking
      .connect(fixture.guest)
      .checkInAttested(1, auth.validAfter, auth.validUntil, auth.nonce, auth.signature)
  ...
  it('uses exact bounded pages without filtering deactivated IDs', async () => {
    const { booking, host, lens } = await deployFixture()
    ...
    const [first, next] = await lens.getListingsPage(0, 2)
    const [second, done] = await lens.getListingsPage(next, 2)
    ...
    await expect(lens.getListingsPage(0, 51)).to.be.revertedWithCustomError(
      lens,
      'InvalidPagination'
    )
  })
