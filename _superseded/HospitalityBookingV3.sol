 ...
 */
contract HospitalityBookingV3 is
  AccessControl,
  Pausable,
  ReentrancyGuard,
  EIP712,
  ERC721
{
  ...
  constructor(
    ...
    address pauser_,
    address arbitrator_
  ) ERC721('Hospitality Stay', 'HSTAY') EIP712('HospitalityBookingV3', '1') {
    if (
  ...
  function checkIn(
    ...
    if (signer != booking.host) revert Unauthorized();
  ...
  function submitReview(
    ...
    if (booking.reviewSubmitted) revert InvalidState();
    if (rating < 1 || rating > 5 || bytes(uri).length > 200 || contentHash == bytes32(0)) {
      revert InvalidInput();
    }
    ...
    _listingReviewBookingIds[booking.listingId].push(bookingId);
  ...
  function getReview(uint256 bookingId) external view returns (Review memory) {
    if (_reviews[bookingId].bookingId == 0) revert NotFound();
  ...
  function getListingsPage(
    uint256 cursor,
    uint256 limit
  ) external view returns (Listing[] memory page, uint256 nextCursor) {
    _validatePage(cursor, limit, _listingCount);
    uint256 end = _pageEnd(cursor, limit, _listingCount);
    page = new Listing[](end - cursor);
    for (uint256 i = cursor; i < end; i++) {
      page[i - cursor] = _listings[i + 1];
    }
    nextCursor = end < _listingCount ? end : 0;
  }

  function getListingRoomTypeIdsPage(
    uint256 listingId,
    uint256 cursor,
    uint256 limit
  ) external view returns (uint256[] memory page, uint256 nextCursor) {
    return _uintPage(_listingRoomTypeIds[listingId], cursor, limit);
  }

  function getListingBookingIdsPage(
    uint256 listingId,
    uint256 cursor,
    uint256 limit
  ) external view returns (uint256[] memory page, uint256 nextCursor) {
    return _uintPage(_listingBookingIds[listingId], cursor, limit);
  }

  function getGuestBookingIdsPage(
    address guest,
    uint256 cursor,
    uint256 limit
  ) external view returns (uint256[] memory page, uint256 nextCursor) {
    return _uintPage(_guestBookingIds[guest], cursor, limit);
  }

  function getHostBookingIdsPage(
    address host,
    uint256 cursor,
    uint256 limit
  ) external view returns (uint256[] memory page, uint256 nextCursor) {
    return _uintPage(_hostBookingIds[host], cursor, limit);
  }

  function getListingReviewBookingIdsPage(
    uint256 listingId,
    uint256 cursor,
    uint256 limit
  ) external view returns (uint256[] memory page, uint256 nextCursor) {
    return _uintPage(_listingReviewBookingIds[listingId], cursor, limit);
  }
  ...
  function _uintPage(
    ...
    uint256 cursor,
    uint256 limit
  ) internal view returns (uint256[] memory page, uint256 nextCursor) {
    _validatePage(cursor, limit, values.length);
    uint256 end = _pageEnd(cursor, limit, values.length);
    page = new uint256[](end - cursor);
    for (uint256 i = cursor; i < end; i++) {
      page[i - cursor] = values[i];
    }
    nextCursor = end < values.length ? end : 0;
  }
